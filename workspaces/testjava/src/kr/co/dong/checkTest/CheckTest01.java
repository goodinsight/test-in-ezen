...
//			System.out.println(romNum[4]);
		...
		// 20형 배열 생성 // 행(아래, 값)길이 전체 romNum[2] 스티링을 int로 intValue1 =
...
//	       		System.out.println(rom40[i][0]); 
		...
		for (int i = 1; i <= rom20.length; i++) {
			rom20[i - 1][1] = Integer.toString(romTmp + i - floor20 * floorCounter); // rom20[i][1]의 갚을 1층에 4개방 20평은 한층에
																						// 4개
			if (i % floor20 == 0) {
				romTmp = romTmp + 100;
				floorCounter++;
		...
		for (int i = 1; i <= rom40.length; i++) {
			...
			rom40[i - 1][1] = Integer.toString(romTmp + i - floor40 * floorCounter); // rom20[i][1]의 갚을 1층에 2개방 40평은 한층에
																						// 방 2개
			if (i % floor40 == 0) {
				romTmp = romTmp + 100;
				floorCounter++;
...
//	       		System.out.println(rom40[i][2]); 
		...
		String[] roomNameRand = { "Nice", "Good", "Like", "Need", "Time", "Life", "Work", "Hope", "Fear", "Rain",
				"Fire", "Snow", "Love", "Mind", "Star", "Rain", "Song", "Wind", "Idea", "Game", "Team", "Luck", "Gift",
				"Book", "Home", "Tree", "Bird", "Hand", "Word", "Pick", "Gold", "VVIP", "Mine" };
		rom20[0][3] = "Gift";
		rom40[0][3] = "Good";
...
//	           		System.out.println(rom40[i][5]); 
		...
		for (int i = 0; i < rom20.length; i++)
			rom20[i][6] = rom20[i][1] + Integer.toString(1002); // rom20[i][0]의 카운트가 0부터가 아니라 1부터 시작하게 i+1 함 단순입력 괄호 생략
																// 주의
																// 방사용여부 구분 예약번호 1002면 20형
		for (int i = 0; i < rom40.length; i++)
			rom40[i][6] = rom40[i][1] + Integer.toString(1004); // rom40[i][0]의 카운트가 0부터가 아니라 1부터 시작하게 i+1 함 단순출력 괄호 생략
																// 주의
...
//	       		System.out.println(rom40[i][6]); 
		...
		while (!power) {
			...
			case 2: // 방예약
				...
				// 관련 변수 romNum[5] 예약가능20평형, romNum[6] 예약가능 40평형, rom20[i][2], rom40[i][2] 사용 O,
				// 미사용 X
				...
				if (inputPass == PASS) {
					...
					while (adminOnOff != 2) {
						...
						case 1:
							...
							for (int i = 0; i < rom20.length; i++) {
								System.out.printf(" %s   %s   %s   %s  %s   %s    %s\n", rom20[i][0], rom20[i][1],
										rom20[i][2], rom20[i][3], rom20[i][4], rom20[i][5], rom20[i][6], rom20[i][7]);
							}
							System.out.println("40평형 현황");
							System.out.println("위치 방번호 예약   방이름   가격      고객명    취소번호   크기");
							for (int i = 0; i < rom40.length; i++) {
								System.out.printf(" %s   %s   %s   %s  %s   %s    %s\n", rom40[i][0], rom40[i][1],
										rom40[i][2], rom40[i][3], rom40[i][4], rom40[i][5], rom40[i][6], rom40[i][7]);
							}
							System.out.println();
							System.out.println("== 예약현황 ==");
							// 관련 변수 romNum[5] 예약가능20평형, romNum[6], 20 예약인원 romNum[7], 40예약인원 romNum[8] 예약가능
							// 40평형
						...
						case 2:
							...
							System.out.printf("1. 20평형 > 예약 %d 건, 매출액 = %7d \n", romNum[7],
									romNum[7] * Integer.parseInt(rom20[0][4])); // 단일값이라 고정, 추후 변경
							System.out.printf("2. 40평형 > 예약 %d 건, 매출액 = %7d \n", romNum[8],
									romNum[8] * Integer.parseInt(rom40[0][4])); // 단일값이라 고정, 추후 변경
							System.out.printf(" 합계 매출은 = 총%d 입니다. \n", romNum[7] * Integer.parseInt(rom20[0][4])
									+ romNum[8] * Integer.parseInt(rom40[0][4])); // 단일값이라 고정, 추후 변경
						...
						case 9:
							...
							System.out.println("                   -- 20형 --                      ");
							for (int i = 0; i < rom20.length; i++) { // 인원수 //
								System.out.printf(" %s호 (%s) 예약(%s_Max %s)  ", rom20[i][1], rom20[i][3], rom20[i][2],
										rom20[i][7]);
								if ((i + 1) % 2 == 0) {
									System.out.println();
									for (int j = 0; j < 2; j++) {
							...
							System.out.println("                   -- 40형 --                      ");
							for (int i = 0; i < rom20.length; i++) {
								System.out.printf(" %s호 (%s) 예약(%s_Max %s)  ", rom40[i][1], rom40[i][3], rom40[i][2],
										rom40[i][7]);
								if ((i + 1) % 2 == 0) {
									System.out.println();
									for (int j = 0; j < 2; j++) {
