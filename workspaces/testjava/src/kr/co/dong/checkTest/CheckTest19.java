...
public class CheckTest19 {
	public static void main(String[] args) {
		Scanner scan = new Scanner(System.in);
		String roomType[][] = { { "스탠다드", "20000", "60000", "2" }, { "디럭스", "25000", "70000", "2" },
				{ "트윈", "30000", "90000", "4" } };
		...
		while (true) {
			...
			case 1:
				for (int i = 0; i < room.length;) {
					...
					System.out.printf(room[i][0] + "번 %4s" + " " + typeName[0] + " : " + room[i][2] + " " + typeName[1]
							+ " : " + room[i][3] + " " + room[i][4] + "인실 " + state, room[i][1]);
					i++;
					if (i % 3 == 0 && i != 0) {
						System.out.println();
					} else {
				...
				if (selNum >= Integer.parseInt(room[0][0]) && selNum <= Integer.parseInt(room[room.length - 1][0])) {
					...
					while (true) {
						...
						if (sel == 0) {
							...
							System.out.println(selNum + "번 방 " + room[tmp][1] + " " + typeName[sel - 1] + " "
									+ room[tmp][sel + 1] + "원 선택");
							System.out.println(room[tmp][1] + "방의 기준 인원은 " + room[tmp][4]
									+ "명 입니다. (1)추가 인원 없음 (2)인원 추가(인원 당 만원 추가) : ");
			...
			case 2:
				...
				while (true) {
					...
					if (selNum2 > Integer.parseInt(room[0][0])
							&& selNum2 < Integer.parseInt(room[room.length - 1][0])) {
				...
				if (pwdinput.equals(pwd)) {
					while (!admin_Power) {
						...
						case 1:
							for (int i = 0; i < room.length;) {
								System.out.printf(
										room[i][0] + "번 %4s" + " " + typeName[0] + " : " + room[i][2] + " "
												+ typeName[1] + " : " + room[i][3] + " " + room[i][4] + "인실 ",
										room[i][1]);
								if (room[i][7].equals("NotReservation")) {
									state = "예약가능";
								}
							...
							System.out.print(room[roomNumtmp][0] + "번 방 " + room[roomNumtmp][1] + " " + typeName[0]
									+ " : " + room[roomNumtmp][2] + " " + typeName[1] + " : " + room[roomNumtmp][3]
									+ " ");
							if (!(room[roomNumtmp][7].equals("Reservation"))) {
								...
								System.out.println(state);
								System.out.println(" 추가 인원 : " + plus + " 예약자명 : " + room[roomNumtmp][5] + " 예약번호 : "
										+ room[roomNumtmp][6]);
							}
							break; // case 1
