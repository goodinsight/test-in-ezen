...
public class CheckTest07 {
	public static void main(String[] args) {
		...
		String[] reservationStatus = { "○예약 가능○", "●예약불가능●" }; // roomStatus[]의 저장되어 있는 값에 따라 출력할 문자열 ex) 0 = 예약가능, 1 =
																// 예약 불가능

		String[][] reservationNumber = new String[roomScale][2]; // 예약자의 이름과 예약번호를 저장 시킬 2차원 배열 [][0] = 이름, [][1] = 예약번호

		// 방의 이름과 가격 투숙가능한 인원수를 저장한 2차원배열
		String[][] roomClass = { { "Deluxe", "Sweet", "Special", "Guest", "Suit", "King" }, // [0][i] 방의 이름
				{ "100,000", "150,000", "200,000", "250,000", "130,000", "300,000" }, // [1][i] 방의 가격
				{ "3", "2", "4", "3", "2", "5" } // [2][i] 투숙가능한 인원수
		};

		// roomNumvers[]의 0 ~ 입력한 배열의 길이까지 호실 번호를 저장 (각 층마다 방은 4개씩 저장하기 위해 호실은 X01 ~ X04
		// 까지)
		...
		hotel: while (true) {
			...
			do {
				...
				for (int j = i - i % 4; j <= i; j++) { // 입력한 배열의 길이가 17 이라면 16-(16%4) = 16-0 = 16 = 초기값 j=i; 입력한 배열의 길이
														// - 1 16까지
					System.out.printf("[ %2d호 %s\t] \t", roomNumbers[j], roomClass[0][i / 4 + 1]); // roomNumbers[16],
																									// roomClass[0=방의이름][입력한
																									// 배열의 길이-1(16)/4+1]
				}
				System.out.println();
			...
			case "1":
				...
				for (int j = 0; j < roomNumbers.length; j++) {
					if (roomNumbers[j] == num) {
						...
						// roomClass[방의 이름][roomNumbers[찾은 호실이 저장되어 있는 인덱스]/100-1 = 찾은 호실의 방의 이름이 저장되어
						// 있는 인덱스]
						System.out.println(
								"선택하신 객실은 " + roomNumbers[j] + "호 " + roomClass[0][roomNumbers[j] / 100 - 1] + "이며");
			...
			case "2":
				...
				for (int j = 0; j < reservationNumber.length; j++) {
					if (input.equals(reservationNumber[j][0])) {
						System.out.println(reservationNumber[j][0] + " 님");
						System.out.println(
								"예약하신 객실은 " + roomNumbers[j] + "호 " + roomClass[0][roomNumbers[j] / 100 - 1] + "입니다.");
