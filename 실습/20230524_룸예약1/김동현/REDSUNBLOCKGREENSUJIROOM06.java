...
public class REDSUNBLOCKGREENSUJIROOM06 {
	...
	public static void main(String[] args) {
		...
		int abc = 0;// 객실인원 과 방이름 과 예약자이름 등등을 보여주기위한 임시변수!?!?
		...
		math = new int[roomsize];// 각각의 변수들에게 룸사이즈배열을 지정하자
		...
		while (!power) {//파워가 구동중일동안 와일드를 실행하하
			...
			case 2:// 취소하기 여부 등등이지
				...
				break;
			case 3:// 방의 인원들 여부 보여주기
				System.out.println("객실인원");
				abc = scan.nextInt();
				System.out.println(hotelnumber[roomsize ]);
				saram[roomsize]=scan.next();
				
				if (summerin[roomsize] >= 0) {// 0이상이면 인원이 존재한다는것
					System.out.println("현재 부합하는 객실인원 여부는:" + summerin[roomsize]);
				} else if (summerin[roomsize] < 0) {// 0미만이면 인원이 없다는 것이지
					System.out.println("객실인원이 존재하지 않네여");
				}
			
				break;
			case 4:// 1이면 방이 예약됐다는 것이고 0이면 방이 비어있다는 것이지
...
//				}
//				System.out.println("방의이름은여??");
//				roomname[roomsize] = scan.next();
//				for (int i = 0; i < roomname[roomsize].length(); i++) {
//					if (roomname.equals(saram)) {// 문자니까 우선 이퀄쓰로 해보자
//						System.out.println("방의이름은 :" + roomname[roomsize]);
//					}
//				}
				System.out.println("예약번호여부는요???");
				math[roomsize]=scan.nextInt();
				if(room[roomsize]==1) {//방의룸사이즈가 1이면 예약이 된거
//					math[roomsize]=1;
					System.out.println("예약번호가정상적으로완료된상태입니당:"+ math[roomsize] );
				}else if(room[roomsize]==0){
					math[roomsize]=0;
					System.out.println("예약을얼른하세요");
				}
				break;
			//case 6:
			...
			case 77: 
				...
				break;
			case 99: //시스템 종료 하기모드 이지
				System.out.println("집에가자이제");
				power = false;
