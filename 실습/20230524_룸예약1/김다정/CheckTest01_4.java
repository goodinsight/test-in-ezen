...
//				        2. 객실인원 입력하기
		...
		int [] price = {60000, 60000, 80000, 80000, 80000, 80000, 80000, 80000, 80000, 120000, 240000, 240000}; // 호실 가격설정
		String [] name = {"싱글룸1", "싱글룸2", "더블룸1", "더블룸2", "더블룸3", "트윈룸1", "트윈룸2", "트윈룸3", "디럭스룸", "패밀리룸", "스위트룸1", "스위트룸2"};  // 호실 이름설정
		int [] num = {1, 1, 2, 2, 2, 2, 2, 2, 2, 8, 10, 10}; // 호실 인원설정
		
		int nameChange1 = 0;       //호수 이름 변경 선택
		String nameChange2 = " ";  //호수 이름 입력 칸 
		
		int numChange1 = 0;    //호수 인원 변경 선택
		int numChange2 = 0;    //호수 인원 변경 입력칸
		
		int priceChange1 = 0;  // 가격 변경 호수 선택
		int priceChange2 = 0;  //호수 가격 변경 입력칸
		
		int people = 0;        // 투숙객 입력칸
		int pay = 0;           // 넣은 돈 입력칸
		
		String [] reserveName = new String [12];   // 예약자 성함
		String [] reserveNum = new String [12];    // 예약시 예약번호
		String [] cancelName = new String [12];    // 취소자 성함
		String [] cancelNum = new String [12];     // 취소시 예약번호
...
//			System.out.println("                                ⑺⑺. 관리자 모드 ");
			...
			case 1:
				...
				if (room[select-1]==1) {
					System.out.println("XXXXXXXXXXXXXXXXXXXXXX > "+select+"호실은 이미 예약이 되어 있어 예약이 불가합니다. XXXXXXXXXXXXXXXX");
					System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 메뉴로 돌아갑니다. 메뉴를 다시 선택해주세요.     XXXXXXXXXXXXXXXX");
					System.out.println();
					break;
				}
				System.out.println("                          > "+select+"호실이 예약이 ★가능★합니다.");
				System.out.println("                          > "+select+"호실의 최대 객실 인원은 ("+num[select-1]+")명 입니다.");
				System.out.println("                          > 투숙하실 인원을 눌러주세요.");
				people = scan.nextInt();
				if (num[select-1]>=people) {
					System.out.println("                          > "+people+" 명 확인 되었습니다.");
				}else {
					System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 최대 객실 인원을 초과 하였습니다. 예약이 불가합니다. XXXXXXXXXXXXXXXXXXX");
					System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 메뉴로 돌아갑니다. 메뉴를 다시 선택해주세요.       XXXXXXXXXXXXXXXXXXX");
					break;
				}
				System.out.println("                          > "+select+"호실의 투숙가격은 "+price[select-1]+" 원 입니다.");
				System.out.println("                          > 결제할 금액을 넣어주세요.");
				pay = scan.nextInt();
				if (price[select-1]<=pay) {
					System.out.println("                          > "+"결제가 확인 되었습니다.");
					System.out.println("                          > "+"거스름돈 "+(pay-price[select-1])+"원이 반환됩니다. " );
				}else {
					System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 금액이 부족합니다. 예약이 불가합니다.         XXXXXXXXXXXXXXXXXXXXXX");
					System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 객실 금액을 확인 후 예약 해주세요.           XXXXXXXXXXXXXXXXXXXXXX");
					System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 메뉴로 돌아갑니다. 메뉴를 다시 선택해주세요.    XXXXXXXXXXXXXXXXXXXXX");
					break;
				}
			...
			case 2:
				...
				if (room[cancel-1]==1) {
					System.out.println("                           > "+cancel+"호실의 취소가 ★가능★합니다.");
					...
					if (reserveName[cancel-1].equals(cancelName[cancel-1])) {
						...
						if (reserveNum[cancel-1].equals(cancelNum[cancel-1])) {
							...
							System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 예약번호가 일치하지 않습니다.            XXXXXXXXXXXXXXXXXXXXXX");
							System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 확인 후 다시 입력해주세요.              XXXXXXXXXXXXXXXXXXXXXX");
							System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 메뉴로 돌아갑니다. 메뉴를 다시 선택해주세요. XXXXXXXXXXXXXXXXXXXXX");
							System.out.println();
							break;
						}
					}else {
						System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 예약자가 목록에 없습니다.              XXXXXXXXXXXXXXXXXXXXXX");
						System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 확인 후 다시 입력해주세요.             XXXXXXXXXXXXXXXXXXXXXX");
						System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 메뉴로 돌아갑니다. 메뉴를 다시 선택해주세요. XXXXXXXXXXXXXXXXXXXXX");
						System.out.println();
						break;
					...
					System.out.println("XXXXXXXXXXXXXXXXXXXXXX > "+cancel+"호실은 예약이 되어있지 않습니다.XXXXXXXXXXXXXXXXXXXXXX");
					System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 확인 후 다시 입력해주세요.              XXXXXXXXXXXXXXXXXXXXXX");
					System.out.println("XXXXXXXXXXXXXXXXXXXXXX > 메뉴로 돌아갑니다. 메뉴를 다시 선택해주세요. XXXXXXXXXXXXXXXXXXXXX");
				}
				break;
