...
public class RoomControl_kdh {
	...
	}// 리스트 내에 해당 내용이 저장되어 있는 index 을 찾는 메써드
	//방번호로 찾을건지?? , 방이름으로 찾을건가??!!
	int findRoom(String name, List<Room> list) {
		int index = -1;//못찾은게 -1이래
		
		for(int i=0; i<list.size();i++) {//리스트룸사이즈 일 동안 반복!?!?!?!?!?!??
//			list.get(i).getrName();   ==> 리스트내에 방이름이지
			if(name.equals(list.get(i).getrName())) {//스트링값이랑 비교해서 알네임이랑 비교해서 부합하는게 있으면 인댁스를 리턴????
				index=-1;
				//return index;
			break;
		}
