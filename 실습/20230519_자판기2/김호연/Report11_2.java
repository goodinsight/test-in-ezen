 ...
 * [_2 변경 사항]
 * 1.변수를 배열로 처리
 * (배열 길이가 더 길어질 것을 고려해 관리자모드 내부 메뉴의 '이전으로' 명령을 9에서 999로 변경)
 * 
				...
				while (true) {
					...
					case 1: // 관리자 모드 - 재고 관리
						while (true) {
							...
							System.out.print("(999)이전으로");
							System.out.printf("%n명령을 입력해주세요 : ");
							input = scan.nextInt();
							if ((input < 1 || input > dName.length) && input != 999) {
								System.out.printf("%n***올바른 숫자를 입력해주세요.***%n");
								continue;
							} else if (input == 999) {
								break;
							...
							System.out.printf("(1)%s 재고 증가 (2)%s 재고 감소 (999)이전으로", dName[idx], dName[idx]);
							System.out.printf("%n명령을 입력해주세요 : ");
							input = scan.nextInt();
					...
					case 4: // 관리자 모드 - 매출 확인
						for (int i = 0; i < dName.length; i++) {
							if (dSell[i] > 0) {
								System.out.printf("%s 판매 수 : %d, 매출 : %d원%n", dName[i], dSell[i], dPrice[i] * dSell[i]);
								tGain += dSell[i] * dPrice[i];
							}
						}
