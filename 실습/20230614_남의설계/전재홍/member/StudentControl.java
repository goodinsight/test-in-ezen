...
public class StudentControl {
	...
	void studentView() {
		...
		System.out.println(" ┌───────────┬───────────┬───────────┬───────────┐");
		System.out.println(" │  1 전체보기 │  2 학번검색 │  3 삭제정보 │  4 돌아가기 │");
		System.out.println(" └───────────┴───────────┴───────────┴───────────┘");
...
//				Student student = students.get(i);
			...
			for(Student sData : students) {// 향상된 for문 | 생성자 생성자 이름 : 리스트 |
		...
		case 2: // 학번검색
			...
			int index;  // 입력하는 학번 (이거 체크 못해서 완전 헤맴)
			index = searchStudents(scan.next()); // 메서드 
	...
	private void addStudent() {
		...
		boolean Same = false; // 학번 중복 확인 || 나이 또는 이름은 중복 될 수 있으나 학번은 중복 되면 안되기 때문에 중복 체크 기능 삽입 
		String num = null; 
		while(!Same){
			System.out.print(" 학번 : ");
			num = scan.next();
			if(students.size()!=0) {
				for(Student nData : students) {
					...
					}else if(students.get(students.size()-1).equals(nData)){ // 0(순서)부터 학적 검색 해야하기 때문 
						Same = true;
					}
				}
			}else {
	...
	boolean updateStudent(int index) {
		...
		System.out.println(" ┌────────────┬───────┬───────┬───────┬───────┐");
		System.out.println(" │ 수정항목 선택 │ 1 이름 │ 2 나이 │ 3 학번 │ 4 전공 │");
		System.out.println(" └────────────┴───────┴───────┴───────┴───────┘");
